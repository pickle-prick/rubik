...
if(type_flags & RK_NodeTypeFlag_TileMapLayer) { node->tilemap_layer = node_bucket->first_free_tilemap_layer; if(node->tilemap_layer != 0) {SLLStackPop(node_bucket->first_free_tilemap_layer);} else {node->tilemap_layer = push_array_no_zero(node_bucket->arena_ref, RK_TileMapLayer, 1);} MemoryZeroStruct(node->tilemap_layer); if(0) {(*node->tilemap_layer) = (RK_TileMapLayer){0};} }\
if(type_flags & RK_NodeTypeFlag_TileMap) { node->tilemap = node_bucket->first_free_tilemap; if(node->tilemap != 0) {SLLStackPop(node_bucket->first_free_tilemap);} else {node->tilemap = push_array_no_zero(node_bucket->arena_ref, RK_TileMap, 1);} MemoryZeroStruct(node->tilemap); if(0) {(*node->tilemap) = (RK_TileMap){0};} }\
