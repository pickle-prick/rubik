...
if(type_flags & RK_NodeTypeFlag_Node2D) { node->node2d = node_bucket->first_free_node2d; if(node->node2d != 0) {SLLStackPop(node_bucket->first_free_node2d);} else {node->node2d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Node2D);} MemoryZeroStruct(node->node2d); if(1) {(*node->node2d) = (RK_Node2D){.transform = {.scale = v2f32(1,1)}};} *(U8**)((U8*)node->node2d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Node3D) { node->node3d = node_bucket->first_free_node3d; if(node->node3d != 0) {SLLStackPop(node_bucket->first_free_node3d);} else {node->node3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Node3D);} MemoryZeroStruct(node->node3d); if(1) {(*node->node3d) = (RK_Node3D){.transform = {.rotation = make_indentity_quat_f32(), .scale = v3f32(1,1,1)}};} *(U8**)((U8*)node->node3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Camera3D) { node->camera3d = node_bucket->first_free_camera3d; if(node->camera3d != 0) {SLLStackPop(node_bucket->first_free_camera3d);} else {node->camera3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Camera3D);} MemoryZeroStruct(node->camera3d); if(0) {(*node->camera3d) = (RK_Camera3D){0};} *(U8**)((U8*)node->camera3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_MeshInstance3D) { node->mesh_inst3d = node_bucket->first_free_mesh_inst3d; if(node->mesh_inst3d != 0) {SLLStackPop(node_bucket->first_free_mesh_inst3d);} else {node->mesh_inst3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_MeshInstance3D);} MemoryZeroStruct(node->mesh_inst3d); if(0) {(*node->mesh_inst3d) = (RK_MeshInstance3D){0};} *(U8**)((U8*)node->mesh_inst3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_SceneInstance) { node->scene_inst = node_bucket->first_free_scene_inst; if(node->scene_inst != 0) {SLLStackPop(node_bucket->first_free_scene_inst);} else {node->scene_inst = push_array_fat_no_zero(node_bucket->arena_ref, RK_SceneInstance);} MemoryZeroStruct(node->scene_inst); if(0) {(*node->scene_inst) = (RK_SceneInstance){0};} *(U8**)((U8*)node->scene_inst-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_AnimationPlayer) { node->animation_player = node_bucket->first_free_animation_player; if(node->animation_player != 0) {SLLStackPop(node_bucket->first_free_animation_player);} else {node->animation_player = push_array_fat_no_zero(node_bucket->arena_ref, RK_AnimationPlayer);} MemoryZeroStruct(node->animation_player); if(0) {(*node->animation_player) = (RK_AnimationPlayer){0};} *(U8**)((U8*)node->animation_player-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_DirectionalLight) { node->directional_light = node_bucket->first_free_directional_light; if(node->directional_light != 0) {SLLStackPop(node_bucket->first_free_directional_light);} else {node->directional_light = push_array_fat_no_zero(node_bucket->arena_ref, RK_DirectionalLight);} MemoryZeroStruct(node->directional_light); if(0) {(*node->directional_light) = (RK_DirectionalLight){0};} *(U8**)((U8*)node->directional_light-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_PointLight) { node->point_light = node_bucket->first_free_point_light; if(node->point_light != 0) {SLLStackPop(node_bucket->first_free_point_light);} else {node->point_light = push_array_fat_no_zero(node_bucket->arena_ref, RK_PointLight);} MemoryZeroStruct(node->point_light); if(0) {(*node->point_light) = (RK_PointLight){0};} *(U8**)((U8*)node->point_light-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_SpotLight) { node->spot_light = node_bucket->first_free_spot_light; if(node->spot_light != 0) {SLLStackPop(node_bucket->first_free_spot_light);} else {node->spot_light = push_array_fat_no_zero(node_bucket->arena_ref, RK_SpotLight);} MemoryZeroStruct(node->spot_light); if(0) {(*node->spot_light) = (RK_SpotLight){0};} *(U8**)((U8*)node->spot_light-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Sprite2D) { node->sprite2d = node_bucket->first_free_sprite2d; if(node->sprite2d != 0) {SLLStackPop(node_bucket->first_free_sprite2d);} else {node->sprite2d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Sprite2D);} MemoryZeroStruct(node->sprite2d); if(0) {(*node->sprite2d) = (RK_Sprite2D){0};} *(U8**)((U8*)node->sprite2d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_AnimatedSprite2D) { node->animated_sprite2d = node_bucket->first_free_animated_sprite2d; if(node->animated_sprite2d != 0) {SLLStackPop(node_bucket->first_free_animated_sprite2d);} else {node->animated_sprite2d = push_array_fat_no_zero(node_bucket->arena_ref, RK_AnimatedSprite2D);} MemoryZeroStruct(node->animated_sprite2d); if(0) {(*node->animated_sprite2d) = (RK_AnimatedSprite2D){0};} *(U8**)((U8*)node->animated_sprite2d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_TileMapLayer) { node->tilemap_layer = node_bucket->first_free_tilemap_layer; if(node->tilemap_layer != 0) {SLLStackPop(node_bucket->first_free_tilemap_layer);} else {node->tilemap_layer = push_array_fat_no_zero(node_bucket->arena_ref, RK_TileMapLayer);} MemoryZeroStruct(node->tilemap_layer); if(0) {(*node->tilemap_layer) = (RK_TileMapLayer){0};} *(U8**)((U8*)node->tilemap_layer-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_TileMap) { node->tilemap = node_bucket->first_free_tilemap; if(node->tilemap != 0) {SLLStackPop(node_bucket->first_free_tilemap);} else {node->tilemap = push_array_fat_no_zero(node_bucket->arena_ref, RK_TileMap);} MemoryZeroStruct(node->tilemap); if(0) {(*node->tilemap) = (RK_TileMap){0};} *(U8**)((U8*)node->tilemap-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Particle3D) { node->particle3d = node_bucket->first_free_particle3d; if(node->particle3d != 0) {SLLStackPop(node_bucket->first_free_particle3d);} else {node->particle3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Particle3D);} MemoryZeroStruct(node->particle3d); if(0) {(*node->particle3d) = (RK_Particle3D){0};} *(U8**)((U8*)node->particle3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_HookSpring3D) { node->hook_spring3d = node_bucket->first_free_hook_spring3d; if(node->hook_spring3d != 0) {SLLStackPop(node_bucket->first_free_hook_spring3d);} else {node->hook_spring3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_HookSpring3D);} MemoryZeroStruct(node->hook_spring3d); if(0) {(*node->hook_spring3d) = (RK_HookSpring3D){0};} *(U8**)((U8*)node->hook_spring3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Constraint3D) { node->constraint3d = node_bucket->first_free_constraint3d; if(node->constraint3d != 0) {SLLStackPop(node_bucket->first_free_constraint3d);} else {node->constraint3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Constraint3D);} MemoryZeroStruct(node->constraint3d); if(0) {(*node->constraint3d) = (RK_Constraint3D){0};} *(U8**)((U8*)node->constraint3d-8) = (U8*)node; }\
if(type_flags & RK_NodeTypeFlag_Rigidbody3D) { node->rigidbody3d = node_bucket->first_free_rigidbody3d; if(node->rigidbody3d != 0) {SLLStackPop(node_bucket->first_free_rigidbody3d);} else {node->rigidbody3d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Rigidbody3D);} MemoryZeroStruct(node->rigidbody3d); if(0) {(*node->rigidbody3d) = (RK_Rigidbody3D){0};} *(U8**)((U8*)node->rigidbody3d-8) = (U8*)node; }\
