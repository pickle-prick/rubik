...
if(type_flags & RK_NodeTypeFlag_Collider2D) { node->collider2d = node_bucket->first_free_collider2d; if(node->collider2d != 0) {SLLStackPop(node_bucket->first_free_collider2d);} else {node->collider2d = push_array_fat_no_zero(node_bucket->arena_ref, RK_Collider2D, node);} MemoryZeroStruct(node->collider2d); if(0) {(*node->collider2d) = (RK_Collider2D){0};}}\
...
if(type_flags & RK_NodeTypeFlag_Collider2D) { SLLStackPush(node_bucket->first_free_collider2d, node->collider2d); node->collider2d = 0; }\
