...
rk_scene_entry__3()
{
    ...
    S3_Scene *scene_data = (root->custom_data = rk_node_push_custom_data(S3_Scene, 1));
    ...
    RK_Parent_Scope(root)
    {
        ...
        grid->node2d->transform.depth = 999;
        grid->sprite2d->color = v4f32(0.9,0.9,0.9,1);
        grid->sprite2d->anchor = RK_Sprite2DAnchorKind_Center;
        rk_node_push_fn(grid, s3_fn_grid);
        S3_Grid *grid_data = (grid->custom_data = rk_node_push_custom_data(S3_Grid, 1));
        grid_data->size = cell_count;
        grid_data->cells = rk_node_push_custom_data(S3_Cell, cell_count.x*cell_count.y);
        ...
        RK_Parent_Scope(grid) for(U64 j = 0; j < cell_count.y; j++)
        {
            ...
            for(U64 i = 0; i < cell_count.x; i++)
            {
                ...
                cell->node2d->transform.depth = 998;
